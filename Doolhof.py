...
def teken_doolhof(scherm, blokjesgrootte, doolhof, draad_locaties, sleutel_locatie):
    for y in range(rijen):
        for x in range(kolommen):
            if doolhof[y][x] == "X":
                scherm.blit(steen, (x*blokjesgrootte, y*blokjesgrootte))
            if (y, x) in draad_locaties:
                scherm.blit(DraadVanAriadne, (x*blokjesgrootte, y*blokjesgrootte))

    if sleutel_locatie is not None:
        ry, rx = sleutel_locatie
        scherm.blit(Sleutel, (rx*blokjesgrootte, ry*blokjesgrootte))
